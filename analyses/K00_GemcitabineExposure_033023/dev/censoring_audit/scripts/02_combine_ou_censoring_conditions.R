...
usage <- paste0(
  "Usage: 02_combine_ou_censoring_conditions.R [options]\n\n",
  "Options:\n",
...
condition_dir <- normalizePath(
  args$condition_dir %||% file.path(audit_dir, "artifacts/ou_censoring_simulation_conditions"),
  mustWork = TRUE
)
out_rds <- normalizePath(
  args$out_rds %||% file.path(audit_dir, "artifacts/ou_censoring_simulation.rds"),
  mustWork = FALSE
)
