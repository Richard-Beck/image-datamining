...
DEFAULT_RUN_DIR = Path("pipelines/fucci_3d_segmentation/runs/fucci_absz_sd2_product_cpsam_3d")
...
def run_cpsam_3d(img: np.ndarray, anisotropy: float, min_size: int, gpu: bool) -> np.ndarray:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run 3D CPSAM for FUCCI fused plus product(abs z-score, SD squared) across raw crop TIFFs."
    )
    ...
    parser.add_argument("--resume", action="store_true", help="Skip samples whose full mask stack and PNG already exist.")
    ...
    config = {
        ...
        "preprocessing": {
            ...
            "cpsam_input_axes": "ZCYX full stack; segmentation uses CellposeSAM do_3D=True",
        },
    ...
    (run_dir / "config.fucci_absz_sd2_product_cpsam_3d.json").write_text(json.dumps(config, indent=2) + "\n")
    ...
    for i, raw_crop_path in enumerate(raw_crops, start=1):
        ...
        mask_path = mask_dir / f"{sample_id}_{INPUT_KIND}_3d_cpsam_masks.tif"
        png_path = png_dir / f"{sample_id}_{INPUT_KIND}_3d_z10-70_2x7_bf_mask.png"
        ...
        try:
            ...
            print(f"[{i}/{len(raw_crops)}] segmenting 3D {sample_id}", flush=True)
            masks = run_cpsam_3d(
                img,
                anisotropy=args.anisotropy,
                min_size=args.min_size,
            ...
            print(f"[{i}/{len(raw_crops)}] wrote {mask_path} and {png_path}", flush=True)
        except Exception:
